...
public interface Manned2Repository extends JpaRepository<Manned2, Integer> {

    @Modifying
    @Query("update Manned2 man set man.name = :name where man.id = :id")
    public void updateName(@Param(value = "id") Integer id, @Param(value = "name") String name);

    @Modifying
    @Query("update Manned2 man set man.power = :power where man.id = :id")
    public void updatePower(@Param(value = "id") Integer id, @Param(value = "power") String power);

    @Modifying
    @Query("update Manned2 man set man.year = :year where man.id = :id")
    public void updateYear(@Param(value = "id") Integer id, @Param(value = "year") Integer year);

    @Modifying
    @Query("update Manned2 man set man.country = :country where man.id = :id")
    public void updateCountry(@Param(value = "id") Integer id, @Param(value = "country") String country);

    @Modifying
    @Query("update Manned2 man set man.ncrew = :ncrew where man.id = :id")
